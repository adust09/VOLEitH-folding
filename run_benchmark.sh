...
echo "Running F2 Single Hash benchmark $RUNS times..."
for i in $(seq 1 $RUNS); do
    echo "Run $i of $RUNS"
    
    # Run the benchmark
    output=$(cargo run -- prove --field f2 2>&1)
    ...
    cp results/metrics/f2_single.json "benchmark_results/run_${i}_metrics.json"
...
cat > benchmark_results/avg_metrics.txt << EOF
