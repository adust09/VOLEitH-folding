...
impl Add for UnreducedGF2p128Fast {
    ...
    fn add(self, other: Self) -> Self::Output {
        Self(unsafe {
            [_mm_xor_si128(self.0[0], other.0[0]), _mm_xor_si128(self.0[1], other.0[1])]
        })
    }
}
    ...
    macro_rules! make_tests_gf2p128 {
        (
            ...
            fn $test_mul_name() {
                ...
                for _ in 0..1000 {
                    let val1 = F::random(thread_rng());
                    assert_eq!(val1 * F::ZERO, F::ZERO, "multiplication with zero is not zero");
                    assert_eq!(val1 * F::ONE, val1, "one is not the identity");
                    let val2 = F::random(thread_rng());
                    assert_eq!(val1 * val2, val2 * val1, "multiplication is not commutative");
                }
