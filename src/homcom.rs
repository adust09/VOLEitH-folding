    ...
    VitHS::Field: VecToGF2p128<GF2p128> + fmt::Debug,
{
    ...
    fn new(num_bit_commitments: usize) -> Self {
        Self { vith_sender: VitHS::new(num_bit_commitments, VitHS::Field::VECTOR_SIZE) }
    }
    ...
    VitHR::Field: VecToGF2p128<GF2p128>,
{
    ...
    fn new(num_bit_commitments: usize) -> Self {
        Self { vith_receiver: VitHR::new(num_bit_commitments, VitHR::Field::VECTOR_SIZE) }
    }
    ...
    fn commit_send_challenge_from_seed(
        ...
        self.vith_receiver.generate_consistency_challenge_from_seed(seed)
    }
...
mod tests {
    ...
    fn test_correctness<F: SmallGF>() {
        ...
        for (b_i, key_i, tag_i) in
            izip!(bits.bits.iter().by_vals(), keys.iter().copied(), tags.iter().copied())
        {
            if b_i {
